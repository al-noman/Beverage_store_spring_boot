...
<div class="container" align="center">
    <form action="confirmOrder.jsp" method="post">
        ...
        <table border="2" class="table table-striped table-responsive">
            ...
            <tr>
                <td>
                    <div class="col-xs-1"><%= item.getOrderItems().getManufacturer() %>
                    </div>
                </td>
                <td>
                    <div class="col-xs-1"><%= item.getOrderItems().getName()%>
                    </div>
                </td>
                <td>
                    <div class="col-xs-1"><%= String.format("%.2f", item.getOrderItems().getPrice())%>
                    </div>
                </td>
                <td>
                    <div class="col-xs-1"><%= item.getOrderItems().getIncentive().isPresent() ? "Name: " + item.getOrderItems().getIncentive().get().getName() : " No incentive"%>
                    </div>
                </td>
                <td>
                    <div class="col-xs-1"><%= item.getOrderAmount()%>
                    </div>
                </td>
            </tr>
            <% } %>
